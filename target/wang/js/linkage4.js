		...
		success : function(lists) {
			...
			for ( var shen in lists) {
				if (lists[shen].name == shengs) {
					allshen += "<option selected='selected' value="
							+ lists[shen].code + ">" + lists[shen].name
							+ "</option>";
					num = lists[shen].code;
				} else {
			...
			success : function(lists) {
				...
				for ( var i in lists) {
					if (shengs == lists[i].name) {
						allshi += "<option selected='selected' value="
								+ lists[i].code + ">" // 给市变量赋值
								+ lists[i].name + "</option>";
						num1 = lists[i].code;
					} else {
	...
	if (num1 != "") {
		$.ajax({
			...
			success : function(lists) {
				...
				for ( var i in lists) {
					if (shengs == lists[i].name) {
						allxian += "<option selected='selected' value="
...
// ------------------------------------------
/*
 * function getKind(){ $.ajax({ url:"rest/getKind", dataType:"JSON",
 * success:function(obj){ $("#nzw_spacies").empty(); var spacies='<option
 * selected value="请选择">请选择</option>'; for(var i in obj){ spacies+='<option
 * value='+obj[0].id+'>&nbsp;&nbsp;'+obj[i].crop_species+'</option>'; }
