...
$("#shen").blur(function() {
	...
	$("#province").html("");
	if(a==0){
		$("#province").html("省不能为空").css("color","red");ok4=false;
	}else{
		$("#province").html("√").css("color","green");ok4=true;
	}
})
function checkharvest_time(){
	var a=$("input[name=harvestTime ]").val();
	$("#harvest_time").html("");
	console.log(a)
	if(a==0){
		$("#harvest_time").html("不能为空").css("color","red");ok5=false;
	}else{
		$("#harvest_time").html("√").css("color","green");ok5=true;
	}
}
function checksampling_time(){
	var a=$("input[name=samplingTime ]").val();
	console.log(a)
	$("#sampling_time").html("");
	if(a==0){
		$("#sampling_time").html("不能为空").css("color","red");ok6=false;
	}else{
		$("#sampling_time").html("√").css("color","green");ok6=true;
	}
}
function checksampling_people(){
	var a=$("input[name=samplingPeople ]").val();
...
function checkpollution_rate(){
	var a=$("input[name=pollutionRate ]").val();
	var cc = isalphanumbers(a)
	$("#pollution_rate").html("");
	if(a==0){
		$("#pollution_rate").html("不能为空").css("color","red");ok10=false;
	}else if  (cc == false){
		$("#pollution_rate").html("字符输入有误").css("color","red");ok1=false;
	}else{
		$("#pollution_rate").html("√").css("color","green");ok10=true;
	}
}
function isalphanumbers(str){  
	//1000以内小数点后有一位
	var result=str.match(/(^[0-9]{1,3}$)|(^[0-9]{1,3}[\.]{1}[0-9]{1,2}$)/); 
	if(result==null) return false; 
	return true;
}
...
$(".addds .toxin_id1").blur(function() {
	var a=$("input[name=toxinCount ]").val();
	var cc = isalphanumbers(a);
	$(".toxin_count").html("");
	if(a==0){
		$(".toxin_count").html("不能为空").css("color","red");ok12=false;
...
function sub(){
	...
	var ss =  $(".edittable-bottom").length;
//	alert(ss);
	if(ss>0){
		console.log(ok1,ok2,ok4,ok7,ok8,ok9,ok10,ok11,ok12,ok13,ok14)
		if(ok1==true&&ok2==true&&ok4==true&&ok7==true&&ok8==true&&ok9==true&&ok10==true&&ok11==true&&ok12==true&&ok13==true&&ok14==true){
			alert("保存成功"); 
			var url = document.getElementById("search_form").action+="?cc="+cc;
			 console.log(ok14,ok13+"1")
			 var aa = [];
		...
		console.log(ok1,ok2,ok4,ok7,ok8,ok9,ok10,ok11,ok12)
		if(ok1==true&&ok2==true&&ok4==true&&ok7==true&&ok8==true&&ok9==true&&ok10==true&&ok11==true&&ok12){
			alert("保存成功");   
			var url = document.getElementById("search_form").action;
			   console.log(ok14,ok13+"2")
				 var aa = [];
					$("select[name='toxin_id']").each(function(){
						var ab=$(this).val();
						console.log(ab)
